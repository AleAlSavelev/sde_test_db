...
select     1 id,(select max(cb) mb from (select book_ref, count(passenger_id) cb from bookings b join tickets t using(book_ref) group by book_ref) f)::text response
union all
...
select * from (
                  select 4 id, book_ref||'|'||passenger_id||'|'||passenger_name||'|'||contact_data response
...
select distinct 12 id,case when rn1<rn2 then city1||'|'||city2 else city2||'|'||city1 end response
from (
         select tt.*
         from tt
...
select * from (
                  ...
                  select 20 id, avg(cnt)::text response
                  from tt ) r
union all
